...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($age_err)) {
        $sql = "INSERT INTO users (username, password, email, phone, age, status) VALUES (?, ?, ?, ?, ?, ?)";  // Add status here
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_password, $param_email, $param_phone, $param_age, $param_status); // Bind status
        $param_username = $username;
        $param_password = password_hash($password, PASSWORD_DEFAULT);
        $param_email = $email;
        $param_phone = $phone;
        $param_age = $age;
        $param_status = 'active'; // Set status to 'active'
        mysqli_stmt_execute($stmt);
