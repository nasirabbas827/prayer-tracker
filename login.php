...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email_err) && empty($password_err)) {
        $sql = "SELECT id, email, password, status FROM users WHERE email = ?";  // Add 'status' column check
        ...
        if (mysqli_stmt_num_rows($stmt) == 1) {
            ...
            if (mysqli_stmt_fetch($stmt)) {
                ...
                if ($status !== 'active') {
                    // User is not active, display error
                    $email_err = "Your account is not active. Please contact support.";
                } else {
                    // password is correct
